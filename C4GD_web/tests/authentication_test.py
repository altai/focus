...
class AuthenticationTestCase(unittest.TestCase):
    ...
    def test_login(self):
        with \
                mock.patch('C4GD_web.utils.neo4j_api_call') as neo4j_api_call, \
                mock.patch('C4GD_web.utils.keystone_obtain_unscoped') as keystone_obtain_unscoped, \
                ...
                mock.patch('flaskext.principal.identity_changed') as identity_changed, \
                mock.patch('flask.session') as session:
            current_app.config = {
                'KEYSTONE_CONF': {
                    'admin_tenant_id': '1'}}
            neo4j_api_call.return_value = self.ODB_GET_USER_RESPONSE
            keystone_obtain_unscoped.return_value = self.KEYSTONE_OBTAIN_UNSCOPED_RESPONSE
            
            self.ODB_GET_USER_RESPONSE[0]['passwordHash'] = create_hashed_password(u'correctpassword')
            self.assertEqual(False, _login('testaccount', u'wrongpassword'))
            self.assertEqual(True, _login('testaccount', u'correctpassword'))
            
            self.ODB_GET_USER_RESPONSE[0]['passwordHash'] = create_hashed_password(u'хорошийпароль')
